...
interface SendGlobalDamageDialogProps {
  open: boolean
...
export function SendGlobalDamageDialog({
  open,
  onOpenChange,
  fromPlayerName,
  otherPlayersCount,
  onSend,
}: SendGlobalDamageDialogProps) {
  const [amount, setAmount] = useState('3')
  const [type, setType] = useState<CommanderDamageType>('other')
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      toast.success('Dano enviado pra todo mundo')
      onOpenChange(false)
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Erro ao enviar dano global')
    } finally {
      setSubmitting(false)
    }
  }
      ...
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {fromPlayerName} ataca todo mundo
          </DialogTitle>
          <DialogDescription>
            Envia uma solicitação pendente pros outros {otherPlayersCount} jogadores da mesa — cada um confirma na
            tela dele.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="global-damage-amount">Dano</Label>
            <Input
              id="global-damage-amount"
              type="number"
              min={1}
              value={amount}
          ...
          {type === 'commander' && (
            <div className="space-y-1.5">
              <Label htmlFor="global-commander-name">Comandante (opcional)</Label>
              <Input
                id="global-commander-name"
                value={commanderName}
