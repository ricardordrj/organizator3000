    ...
    <Card size="sm" className="gap-0 py-0">
      <div className="flex items-center gap-3 p-3">
        ...
        >
          ...
          {task.kind === 'full' && task.externalRef && (
            <span className="text-xs text-muted-foreground">#{task.externalRef}</span>
          )}
        </button>
        {isDone ? (
      ...
      {expanded && (
        <div className="space-y-1 border-t px-3 py-2 text-sm text-muted-foreground">
          ...
          {task.kind === 'full' && task.assignee && <p>Responsável: {task.assignee}</p>}
          {task.kind === 'full' && task.reporter && <p>Aberto por: {task.reporter}</p>}
