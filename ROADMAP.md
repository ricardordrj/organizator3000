...
- [ ] **Mesão de Commander — contador de vida em `/mesao`** — pra usar direto no celular durante as partidas com Marchesi, Gabigol, Serjão, Daniel e Ricardo (uso esperado: 1-2x/mês). Dinâmica decidida:
  ...
  - [x] **Ranking / leaderboard por temporada**: ao encerrar o mesão, define-se a ordem de colocação (pré-ordenada por vida, 1º = venceu) e isso vira pontos — 1º = 3 pts, 2º = 2 pts, 3º = 1 pt. Botão "Ranking" mostra a classificação da temporada ativa (pontos, vitórias, jogos) com seletor pra ver anos anteriores. Botão "Reiniciar board" arquiva a temporada atual (fica visível no histórico) e abre uma zerada (nome padrão = ano atual) — normalmente reiniciado por ano. Modelado com `commander_seasons` (o board) + `commander_games.season_id` + `commander_game_players.placement`; dá pra encerrar sem ranquear, e nesse caso a partida não pontua.
